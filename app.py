...
BANNER = r"""
┌────────────────────────────────────────────┐
│ 🚀 Stream OTP BOT (updated v5.1)            │
│ ✨ Modern UI • AddAdmin • 3min AutoDelete  │
└────────────────────────────────────────────┘
"""
print(BANNER)
...
async def start_handler(message: Message):
    ...
    kb = []
    if not rows:
        kb = [[InlineKeyboardButton("📦 No stock available", callback_data="no_stock")]]
        text = "📦 No stock available. Admin can upload CSV with /admin."
    else:
        for country, flag, cnt in rows:
            kb.append([InlineKeyboardButton(f"{flag} {country} — {cnt}", callback_data=f"country|{country}")])
        kb.append([InlineKeyboardButton("❓ Help", callback_data="help")])
        text = "🌍 Select a country to get a number:"
...
async def cmd_addadmin(message: Message):
    ...
    if not await admin_only_check(sender.id):
        await message.reply("❌ Not authorized to add admins.")
    ...
    try:
        ...
        await message.reply(f"✅ Added admin: {target_id}")
...
async def cmd_removeadmin(message: Message):
    sender = message.from_user
    if not await admin_only_check(sender.id):
        await message.reply("❌ Not authorized to remove admins.")
    ...
    try:
        ...
        global ADMINS
        ADMINS = [r[0] for r in rows]
        await message.reply(f"✅ Removed admin: {target_id}")
...
async def callback_handler(callback: CallbackQuery):
    ...
    if now - last < COOLDOWN_SECONDS:
        ...
        await callback.answer(f"⏳ Wait {wait}s...", show_alert=True)
        return
    ...
    if data.startswith("country|"):
        ...
        if not row:
            await callback.edit_message_text("📦 No stock available for this country.")
            return
        ...
        display = (
            f"✨ *Your Virtual Number is Ready!*\n\n"
            f"📞 *Number:* `{format_phone(number)}`\n"
            f"🌍 *Country:* {country} {flag}\n"
            f"⏱️ *Issued:* {datetime.utcnow().strftime('%d-%b-%Y %H:%M')} UTC\n\n"
            f"🔐 Use this number to receive OTP instantly."
        )
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton("🌎 Change Country", callback_data="back_home"),
             InlineKeyboardButton("🔁 New Number", callback_data=f"change|{country}")],
            [InlineKeyboardButton("✉️ Get OTP", url=channel_link or CHANNEL_LINK)]
        ])
    ...
    if data.startswith("change|"):
        ...
        if not new_row:
            await callback.edit_message_text(f"❌ No more free numbers for {country}.")
            return
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton("🌎 Change Country", callback_data="back_home"),
             InlineKeyboardButton("🔁 New Number", callback_data=f"change|{country}")],
            [InlineKeyboardButton("✉️ Get OTP", url=CHANNEL_LINK),
...
ADMIN_KB = InlineKeyboardMarkup(inline_keyboard=[
    [InlineKeyboardButton("📤 Upload CSV/XLSX/TXT", callback_data="admin|upload"),
     InlineKeyboardButton("📦 View Stock", callback_data="admin|view")],
    [InlineKeyboardButton("➕ Add Number", callback_data="admin|add"),
...
async def admin_callback_handler(callback: CallbackQuery):
    ...
    user = callback.from_user
    if not await admin_only_check(user.id):
        await callback.edit_message_text("❌ Not authorized.")
        return
    if data == "admin|upload":
        dispatcher_data.setdefault("awaiting_csv", set()).add(user.id)
        await callback.edit_message_text("📤 Send the CSV / TXT / XLSX file now (attach as file).")
        return
    if data == "admin|view":
    ...
    if data == "admin|stats":
        ...
        await callback.edit_message_text(f"📊 Stats\nTotal: {total}\nUsed: {used}\nFree: {free}", reply_markup=ADMIN_KB)
        return
    if data == "admin|add":
        dispatcher_data.setdefault("awaiting_manual_add", set()).add(user.id)
        await callback.edit_message_text("➕ Send a single number (e.g. +254700123456)")
        return
    if data == "admin|delete":
        ...
        await callback.edit_message_text("🗑️ Select country to delete all numbers:", reply_markup=kb)
        return
    if data.startswith("admin|delete|"):
        ...
        await callback.edit_message_text(f"🗑️ Deleted {c} numbers for {country}", reply_markup=ADMIN_KB)
        return
    if data == "admin|logs":
...
async def document_handler(message: Message):
    user = message.from_user
    if not await admin_only_check(user.id):
        await message.reply("❌ Not authorized to upload.")
        return
    ...
    if lower_name.endswith(".txt"):
        try:
            ...
            await message.reply(f"✅ Imported {len(rows_to_insert)} numbers from TXT.")
    ...
    if lower_name.endswith(".xlsx") or lower_name.endswith(".xls"):
        if not OPENPYXL_AVAILABLE:
            await message.reply("❌ XLSX support not available on this server. Install openpyxl.")
            dispatcher_data.get("awaiting_csv", set()).discard(user.id)
        ...
        try:
            ...
            if rows_to_insert:
                await db_many_insert_numbers(rows_to_insert)
            await message.reply(f"✅ Imported {len(rows_to_insert)} numbers from XLSX.")
    ...
    summary_lines = [f"✅ Imported {processed} numbers. (Skipped {skipped})"]
    if processed:
        counts = {}
        ...
        for (country, flag), cnt in sorted(counts.items(), key=lambda x: -x[1]):
            summary_lines.append(f"{flag} {country} — {cnt}")
    if unknown_examples:
        summary_lines.append("\n⚠️ Unknown prefixes (examples): " + ", ".join(list(unknown_examples)[:10]))
...
async def text_handler(message: Message):
    ...
    if user.id in dispatcher_data.get("awaiting_manual_add", set()) and await admin_only_check(user.id):
        ...
        if not n or len(n) < 6:
            await message.reply("❌ Can't parse number. Send e.g. +254700123456")
            return
        country, flag = detect_country_by_prefix(n)
        await db_many_insert_numbers([(country, flag, n, CHANNEL_LINK)])
        await message.reply(f"✅ Added {format_phone(n)} to {country} {flag}")
